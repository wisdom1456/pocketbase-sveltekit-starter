...
import { c as create_ssr_component, b as escape, e as each, a as add_attribute } from "./ssr.js";
...
const LoginForm = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<form class="flex max-w-md flex-col gap-4 rounded-box bg-base-200 p-6">${passwordLogin ? `<h1 class="self-center text-3xl font-bold">${escape(activeTab === "SignIn" ? "Log in" : "Create an account")}</h1> <div class="tabs self-center"><button class="${"tab-bordered tab " + escape(activeTab === "SignIn" ? "tab-active" : "", true)}" aria-label="Log in" type="button">Log in</button> <button class="${"tab-bordered tab " + escape(activeTab === "SignUp" ? "tab-active" : "", true)}" aria-label="Sign up">Sign up</button></div> ${function(__value) {
    ...
    return function(methods) {
      return ` <div class="space-y-2">${each(methods.authProviders, (p) => {
        return `<button class="btn btn-neutral w-full" type="button"><i class="fa-brands fa-google mr-2 text-primary"></i> ${escape(activeTab === "SignIn" ? "Log in" : "Sign up")} with ${escape(p.name)} </button>`;
      })}</div> `;
    }(__value);

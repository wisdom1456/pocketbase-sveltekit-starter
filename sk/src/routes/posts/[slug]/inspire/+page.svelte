<script lang="ts">
  ...
  let processedBody = '';
  let loading = writable(false); // Store to manage loading state
  ...
  onMount(async () => {
    try {
      ...
      if (post) {
        ...
        console.log('Fetched Subposts:', subposts); // Console log for debugging
      } else {
  ...
  async function handleGenerateIdea() {
    if (post) {
      loading.set(true); // Set loading state to true
      try {
        const newSubpost: GenerateBlogResponse = await generateBlogResponse(
          getSeedPrompt(),
          post.id,
          authModel
        );
        if (newSubpost && newSubpost.slug) {
          // Create subpost with correct userid
          const createdSubpost = await createSubpost({
            title: newSubpost.title,
            content: newSubpost.content,
            post: post.id,
            slug: newSubpost.slug,
          });
          if (createdSubpost) {
            goto(`/subposts/${createdSubpost.slug}`);
          } else {
            console.error('Failed to create new subpost');
          }
        } else {
          console.error('Failed to generate subpost');
        }
      } catch (error) {
        console.error('Error generating subpost:', error);
      } finally {
        loading.set(false); // Set loading state to false
      }
    } else {
